...
def naive(numb_string): # наивный поиск
    ...
    for i in range(0, len(numb_string) - 1):
        count_numb[int(numb_string[i] + numb_string[i + 1])] += 1
...
def alg(numb_string):
    ...
    for i in range(len(numb_string)-1):
        Hash = unique_numb.index(int(numb_string[i]))*len_alphabet**(1) + unique_numb.index(int(numb_string[i+1]))*len_alphabet**(0)
...
for i in range(len(hashList)):
    if hashList[i] > 9:
        ...
        NewItemHasList = alphabet.index(int(timeI[0]))*10**1 + alphabet.index(int(timeI[-1]))*10**0
